...
use jrothert\Service\PasswordService;
class PasswordController {
	...
	public function sendForgotPasswordEmail($data) {
		...
		if ($this->passwordService->tryCreateNewPasswordReset ( $data ["email"], $resetToken )) {
			$this->mailer->send ( \Swift_Message::newInstance ( "Reset Password" )->setFrom ( [ 
					'noreply@personalTrainer.com' 
			] )->setTo ( [ 
					$data ["email"] 
			] )->setBody ( '<html>' . ' <head></head>' . ' <body>' . ' <p></p>' . '<a href="https://' . $_SERVER ["HTTP_HOST"] . '/resetpassword/' . $resetToken . '">Reset Password</a>' . ' </body>' . '</html>', 'text/html' ) );
	...
	public function resetPassword($data) {
		...
		if ($data ['password'] == $data ['confirmPassword']) {
			...
			if ($this->passwordService->tryResetPassword ( $data ['resetToken'], password_hash ( $data ['password'], PASSWORD_DEFAULT ) )) {
				...
				echo $this->template->render ( "resetPassword.html.twig", [
						"errorMessage" => "Your reset link has expired"
				] );
			}
		} else {
			echo $this->template->render ( "resetPassword.html.twig", [
